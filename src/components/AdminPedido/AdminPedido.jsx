...
export default function AdminPedido({ pedido }) {
  ...
  const { handleCompletarPedido, handleCobrarPedido, disabled } = useUmamy();
  ...
  return (
    <article className='admin-pedido'>
      <h3>Pedido #{id}</h3>
      <p className='admin-pedido__p'>Cliente: <span className='admin-pedido__span'>{user.name}{user.telefono ? ` - ${user.telefono}` : ''}</span></p>
